...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.d_inp = default(d_inp, 64)
        self.d_out = default(d_out, self.d_inp)
        self.d_head = d_head  # Add this line
        self.n_head = n_head  # Add this line
    ...
    def forward(
        ...
        attn = scaled_dot_product_attention(q, k, v,
            attn_mask=mask,
            is_causal=self.causal,  # 너가 디폴트로 실행됨
...
class SpatialAttention(Attention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embed = RotaryEmbedding(self.d_head, kind='2d') if embed else nn.Identity()
...
class TemporalAttention(Attention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embed = RotaryEmbedding(self.d_head, kind='1d') if embed else nn.Identity()
    ...
    def forward(
        ...
        pattern = 'b c t h w'
        inp = rearrange(video, f'{pattern} -> b h w t c')  # [1, 32, 32, 64, 512]
        b, h, w, *_ = inp.shape
        inp, ps = pack([inp], '* t c') # [1024, 64, 512]
        ...
        out = unpack(out, ps, '* t c')[0]  # [1, 32, 32, 64, 512]
        return rearrange(out, f'b h w t c -> {pattern}')  # [1, 512, 64, 32, 32]
...
class SpaceTimeAttention(nn.Module):
    ...
    def __init__(
        ...
        # Compute channel sizes for space/time heads and explicit in/out channels
        space_hid = d_head[0] * n_head[0]
        time_hid  = d_head[1] * n_head[1]

        # Input channels to the block: if d_inp provided use it, otherwise
        # use the spatial head size (space_hid). Out channels default to time_hid.
        in_channels = default(d_inp, space_hid)
        out_channels = default(d_out, time_hid)
        ...
        # Temporal attention consumes the output of the spatial attention
        # so set its input dim to `space_hid` to keep LayerNorm shapes consistent.
    ...
    def forward(
        ...
        space_cond, time_cond = cond  # (None, None)
        ...
        video = self.space_attn(video, cond=space_cond, mask=mask) + self.space_skip(video)  # torch.Size([1, 512, 64, 32, 32])
        video = self.temp_attn (video, cond=time_cond , mask=mask) + self.time_skip (video)  # torch.Size([1, 512, 64, 32, 32])
        video = self.ffn(video) + self.ffn_skip(video)  # torch.Size([1, 512, 64, 32, 32])
